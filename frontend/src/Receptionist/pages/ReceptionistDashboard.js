                    ...
                    if (response.ok) {
                        ...
                        const detailedAppointments = await Promise.all(
                            pendingAppointments.map(async (appointment) => {
                                ...
                                return {
                                    ...appointment,
                                    doctor: doctorResponse?.data?.doctor?.docName || 'Unknown Doctor',
                                    patient: patientResponse?.data?.patient?.patientName || 'Unknown Patient',
                                    patientContact: patientResponse?.data?.patient?.patientContact || 'Unknown Contact',
                                };
                            })
                        );
    ...
    return (
        <div>
            ...
            <div className="main-recep_table"> 
                ...
                <h3>Pending Appointments List</h3> 
                        ...
                        <ul>
                            {appointments.map((appointment) => (
                                <li key={appointment._id}>
                                    <span><b>Doctor:</b> {appointment.doctor}</span>
                                    <span><b>Patient:</b> {appointment.patient}</span>
                                    <span><b>Contact:</b> {appointment.patientContact}</span>
                                    <span><b>Time:</b> {appointment.time}</span>
                                    <button
                                        className='schedule_app' 
                                        type='button'
